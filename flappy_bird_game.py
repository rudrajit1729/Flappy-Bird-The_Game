...
class Bird:
    ...
    def jump(self):
        ...
        :return: None
        """
        if self.y >72:# stop from going out of screen up
            #print (self.y)
            self.vel = -10.5
            self.tick_count = 0
            self.height = self.y
    ...
    def move(self):
        ...
        # terminal velocity
        if displacement >= 20:
            displacement = (displacement/abs(displacement)) * 20
...
def end_screen(win):
    ...
    """
    run = True
    game_label = END_FONT.render ("  Game Over :(",1,(255,0,20))
    ...
    while run:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                main(win)
        win.blit(game_label, (WIN_WIDTH/2 - text_label.get_width()/2, 150))
...
def main(win):
    ...
    start = False
    lost = False
    game_label = END_FONT.render("        Flappy Bird!!", 1, (255, 215, 0))
    text_label = STAT_FONT.render("Press Space to Start ,Q to quit", 1, (255, 255, 255))
    run = True
    while run:
